...
还可以免费开店哦";
                ...
                if(!empty($ticket)){
	                $url = "http://wx.huipu.org/my/jiangli/index?uid=".$uid."&openid=".$openid."&event=".$event."&time=".$time."&ticket=".$ticket ;
                ...
                $content .= (!empty($object->EventKey))?("\n\n来自用户（".str_replace("qrscene_","",$object->EventKey)."）分享的二维码\nTA已收到".$out."元的奖励\n\n<a href='http://wx.huipu.org/my/share/'>我也要奖励，点此去分享</a>"):"";
            ...
            case "unsubscribe":
                ...
                break;
            case "SCAN":
                $content = "欢迎回来。感谢用户（".$object->EventKey."）的分享";
                break;
            case "CLICK":
...
暂未完善" ;
                    ...
                    case "KEFU":
                    	$content = array();
                    	$content[] = array("Title"=>"联系客服", "Description"=>"点击唤醒QQ，进行咨询", "PicUrl"=>"http://wx.huipu.org/public/kefu.jpg", "Url" =>"http://wpa.qq.com/msgrd?v=3&uin=670799251&site=qq&menu=yes");
                    	break;
...
/:<O>是一个很帅的小伙子";
            ...
            else if (strstr(strtolower($keyword), "哈哈") || strstr($keyword, "呵呵"))
            {
                $content ="笑什么笑";
           }
           else if (strstr(strtolower($keyword), "优惠券") || strstr($keyword, "优惠"))
           {
           	$content = array();
           	$content[] = array("Title"=>"免费领取0.88元优惠券（满2元可用）",  "Description"=>"免费领取0.88元优惠券（满2元可用）", "PicUrl"=>"http://wx.huipu.org/public/coupon/coupon.jpg", "Url" =>"http://wx.huipu.org/mobile/activity/coupon_list");
           }
...
<MsgType><![CDATA[transfer_customer_service]]></MsgType>
    ...
    private function logger($log_content)
    {
        ...
        if(isset($_SERVER['HTTP_APPNAME'])){   //SAE
            ...
            $log_filename = "baixiao_live_log.txt";
            if(file_exists($log_filename) and (abs(filesize($log_filename)) > $max_size)){unlink($log_filename);}
            file_put_contents($log_filename, $log_content."\r\n", FILE_APPEND);
            
            
           $data = simplexml_load_string($log_content,'SimpleXMLElement',LIBXML_NOCDATA);
            
            
          $res = json_encode($data,JSON_UNESCAPED_UNICODE);
            
            $url = "http://www.lang100.com/mp/index/log_content/".$res ;
